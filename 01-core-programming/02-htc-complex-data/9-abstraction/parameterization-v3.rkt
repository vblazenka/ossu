...
(check-expect (map string-length (list "vedran" "marta")) (list 6 5))
            
;; (T -> X) (listof T) -> (listof X)
(define (map2 fn lon)
...
;; ( T -> Boolean) (listof T) -> (listof T)
;; filter list to contain only those elements for which pred procues true
